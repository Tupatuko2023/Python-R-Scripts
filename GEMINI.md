...
Tämä tiedosto määrittelee yleiset säännöt ja kontekstin Gemini CLI -agentille koko repositorion (`Python-R-Scripts`)
tasolla.

## 1) Repositorion rakenne ja skooppi

Tämä repo koostuu useista itsenäisistä tutkimusprojekteista. Agentin on tunnistettava, missä projektissa se kulloinkin
työskentelee:
...
1. **Projektikohtainen ohje**: Jos olet tekemässä muutoksia alikansioon (esim. `Fear-of-Falling/`), noudata kyseisen
   kansion `GEMINI.md` tai `AGENTS.md` -tiedostoa.
...
1. **Data Safety**: Raakadata on muuttumatonta (immutable). Älä koskaan muokkaa `data/`-kansioiden alkuperäisiä
   tiedostoja.
...
* **Analyysi ennen toimintaa**: Käytä `codebase_investigator` -sub-agenttia, jos tehtävä vaatii useiden tiedostojen
  välisen logiikan ymmärtämistä.
* **Ytimekkyys**: CLI-agenttina vältä turhaa puhetta. Keskity koodiin ja selkeisiin suunnitelmiin.
* **Verifiointi**: Aja aina projektin omat testit tai smoke-testit (`npm test`, `pytest`, `Rscript run_smoke_test.R`)
  muutosten jälkeen.
