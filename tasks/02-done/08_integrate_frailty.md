...
2. **Link**: Update the Aim 2 build process (`scripts/10_build_panel_person_period.R` or the Python builder) to merge this Frailty Score into `aim2_panel.csv`.
   - _Key Check_: If the sibling project produces a CSV (e.g., `frailty_scores.csv`), read it from `DATA_ROOT` (assuming shared data root) or simulate the merge if it's code-only.
...
2. **Data Policy**: Do NOT copy raw data from the other project into this repo. Assume the _output_ of that script is available in `DATA_ROOT/derived/` or similar.
