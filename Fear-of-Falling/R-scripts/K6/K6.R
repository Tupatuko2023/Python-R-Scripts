...
# 2: Data preprocessing and variable coding ----------------------------
...
table(analysis_data_raw$kaatumisenpelkoOn, useNA = "ifany")

# Define FOF_status factor (non_FOF vs FOF) ----------------------------
...
analysis_data <- analysis_data %>%   # HUOM: ei analysis_data_raw
  mutate(
...
ggsave(
  filename = file.path(outputs_dir, "K6_FOF_PainVAS0_G2_emmeans.png"),
  plot     = p_pain,
  width    = 7,
  height   = 5,
  dpi      = 300
...
ggsave(
  filename = file.path(outputs_dir, "K6_FOF_SRH3_emmeans.png"),
  plot     = p_srh,
  width    = 7,
  height   = 5,
  dpi      = 300
...
ggsave(
  filename = file.path(outputs_dir, "K6_FOF_SRM3_emmeans.png"),
  plot     = p_srm,
  width    = 7,
  height   = 5,
  dpi      = 300
...
ggsave(
  filename = file.path(outputs_dir, "K6_FOF_contrasts_forest.png"),
  plot     = p_forest,
  width    = 7,
  height   = 5,
  dpi      = 300
...
main_results_path <- file.path(outputs_dir, paste0(script_label, "_main_results.csv"))
