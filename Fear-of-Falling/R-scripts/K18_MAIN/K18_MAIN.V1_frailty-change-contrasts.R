...
#
# Outputs + manifest:
# - script_label: K18_MAIN (canonical)
# - outputs dir: R-scripts/K18_MAIN/outputs/  (resolved via init_paths(script_label))
...
message("01) Loading analysis data with frailty variables from K15_MAIN...")
...
if (!exists("analysis_data")) {
  ...
  if (!file.exists(k15_rdata)) {
    stop("K15_MAIN output not found: ", k15_rdata,
         "\nRun K15_MAIN to generate K15_frailty_analysis_data.RData.")
  }
  load(k15_rdata)
...
if (!("frailty_cat_3_obj" %in% names(analysis_data))) {
  analysis_data <- analysis_data %>% mutate(frailty_cat_3_obj = NA_character_)
}
if (!("frailty_cat_3_2plus" %in% names(analysis_data))) {
  analysis_data <- analysis_data %>% mutate(frailty_cat_3_2plus = NA_character_)
}
...
append_manifest(
  manifest_row(script = getOption("fof.script"),
               ...
               path   = get_relpath(docx_primary_path),
               kind   = "table_docx",
...
append_manifest(
  manifest_row(script = getOption("fof.script"),
               ...
               path   = get_relpath(docx_exploratory_path),
               kind   = "table_docx",
               n      = nrow(analysis_data)),
  getOption("fof.manifest_path")
)
...
append_manifest(
  manifest_row(
    ...
    path   = get_relpath(file.path(output_dir, "K18_PRIMARY_contrasts_M1.png")),
    kind   = "plot_png",
    n      = nrow(analysis_data)
  ),
  getOption("fof.manifest_path")
...
append_manifest(
  manifest_row(
    ...
    path   = get_relpath(file.path(output_dir, "K18_EXPLORATORY_contrasts_M2.png")),
    kind   = "plot_png",
    n      = nrow(analysis_data)
  ),
  getOption("fof.manifest_path")
...
append_manifest(
  manifest_row(
    ...
    path   = get_relpath(file.path(output_dir, "K18_predicted_trajectories_M1.png")),
    kind   = "plot_png",
    n      = nrow(analysis_data)
  ),
  getOption("fof.manifest_path")
...
append_manifest(
  manifest_row(
    ...
    path   = get_relpath(results_en_path),
    kind   = "text",
    n      = nrow(analysis_data)
  ),
  getOption("fof.manifest_path")
...
append_manifest(
  manifest_row(
    ...
    path   = get_relpath(results_fi_path),
    kind   = "text",
    n      = nrow(analysis_data)
  ),
  getOption("fof.manifest_path")
