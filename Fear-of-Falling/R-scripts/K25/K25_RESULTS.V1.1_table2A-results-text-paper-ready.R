#!/usr/bin/env Rscript
# ==============================================================================
# K25_RESULTS - Paper-ready Results text from K24 Table 2A paper-ready CSV
# File tag: K25_RESULTS.V1.1_table2A-results-text-paper-ready.R
# Purpose: Deterministic table-to-text generator (style refinement only).
#
# Input default:
# - R-scripts/K24/outputs/K24_TABLE2A/table2A_paper_ready_v1_1.csv
#
# Outputs (default narrative mode):
...
if (!cli$style %in% c("list", "narrative")) {
  stop("Unsupported --style. Use --style=list or --style=narrative")
}
...
if (cli$style == "narrative") {
  default_md <- file.path(outputs_dir, "results_table2A_from_K24_v1_1_narrative.md")
  default_txt <- file.path(outputs_dir, "results_table2A_from_K24_v1_1_narrative.txt")
} else {
  default_md <- file.path(outputs_dir, "results_table2A_from_K24_v1_1_list.md")
  default_txt <- file.path(outputs_dir, "results_table2A_from_K24_v1_1_list.txt")
}
out_md <- if (!is.na(cli$out_md) && nzchar(cli$out_md)) cli$out_md else default_md
out_txt <- if (!is.na(cli$out_txt) && nzchar(cli$out_txt)) cli$out_txt else default_txt
...
fof_fragments <- vapply(seq_len(nrow(tab2)), function(i) {
  r <- tab2[i, ]
  paste0(
    as.character(r$Outcome), ": ",
    "beta ", as.character(r$FOF_Beta_CI), ", p=", r$p_fof_fmt,
    ", Model_N=", r$Model_N,
...
if (cli$style == "narrative") {
  para1 <- headline
  para2 <- paste0(
    "Outcome-specific estimates were: ",
...
md_lines <- c(
  ...
  if (cli$style == "narrative") "", para1,
  "", para2,
  "", para3,
  if (!is.na(para4)) c("", para4)
)
...
append_artifact(
  label = paste0("results_table2A_from_K24_v1_1_", cli$style, "_md"),
  kind = "doc_md",
  path = out_md,
  n = nrow(tab2),
  notes = "K25 V1.1 paper-ready results text from K24 paper-ready Table 2A CSV"
)
append_artifact(
  label = paste0("results_table2A_from_K24_v1_1_", cli$style, "_txt"),
  kind = "text",
  path = out_txt,
  n = nrow(tab2),
  notes = "K25 V1.1 plain-text results narrative from K24 paper-ready Table 2A CSV"
)
...
append_artifact(
  ...
  kind = "sessioninfo",
  path = session_path,
  notes = "K25 V1.1 sessionInfo + renv diagnostics"
)
