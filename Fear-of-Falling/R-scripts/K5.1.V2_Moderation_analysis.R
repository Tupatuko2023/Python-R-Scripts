...
# Full quartile model including Composite_Z0 main effect
...
# Parsimonious model without Composite_Z0 main effect (to avoid redundancy)
...
p_lsmeans <- ggplot(emm_quart_df,
  ...
  theme_minimal()
ggplot2::ggsave(
  filename = file.path("outputs", "discrete_lsmeans_plot_updated.png"),
...
# Linear vs spline moderator effect (main and interaction)
...
# Predictions from spline model over moderator grid
z_seq <- seq(min(analysis_data_cc$cComposite_Z0),
             max(analysis_data_cc$cComposite_Z0),
             length.out = 100)

# Valitaan "tyypillinen" sukupuoli esim. yleisimmän tason mukaan
...
# Define indicator for complete-case status
...
# Key p-values from continuous model
tidy_main <- tidy_jn_c
...
save_table_csv_html(eta_sq_df, "effectsizes_partial_eta_sq")

# Marginal R2 (käytetään broom::glance)
r2_glance <- broom::glance(model_jn_c)
r2_df <- r2_glance %>%
...
save_table_csv_html(r2_df, "model_jn_c_r2")


# Hedges g for FOF 1 vs 0 at mean cComposite_Z0
emm_mean <- emmeans::emmeans(model_jn_c,
                             specs = "FOF_status",
                             at = list(cComposite_Z0 = 0))
contrast_mean <- emmeans::contrast(emm_mean, method = "revpairwise")
contrast_mean_df <- as.data.frame(contrast_mean)

# Approximate Hedges g from t and group sizes
t_val <- contrast_mean_df$t.ratio[1]
df_res <- contrast_mean_df$df[1]
n_table <- table(analysis_data_cc$FOF_status)
n1 <- as.numeric(n_table["1"])
n0 <- as.numeric(n_table["0"])

g_val <- effectsize::t_to_d(t = t_val, df_error = df_res,
                            n1 = n1, n2 = n0, hedges.correction = TRUE)
g_df <- as.data.frame(g_val)
save_table_csv_html(g_df, "effectsize_hedges_g_fof_mean")

# 9. Subgroup probes and simple-slope contrasts ------------------------------

# Cutpoints: mean, mean ± 0.5 SD, quartile medians
m_z <- mean(analysis_data_cc$cComposite_Z0)
sd_z <- stats::sd(analysis_data_cc$cComposite_Z0)
q_vals <- stats::quantile(analysis_data_cc$cComposite_Z0, probs = c(0.25, 0.5, 0.75))

cutpoints <- c(
  mean = m_z,
  mean_minus_0_5sd = m_z - 0.5 * sd_z,
  mean_plus_0_5sd = m_z + 0.5 * sd_z,
  q25 = q_vals[1],
  q50 = q_vals[2],
  q75 = q_vals[3]
)

emm_cut <- emmeans::emmeans(
  model_jn_c,
  specs = "FOF_status",
  at = list(cComposite_Z0 = unname(cutpoints)),
  by = "cComposite_Z0"
)
emm_cut_df <- as.data.frame(emm_cut)
save_table_csv_html(emm_cut_df, "emm_fof_by_moderator_cutpoints")

contrast_cut <- emmeans::contrast(emm_cut, method = "revpairwise")
contrast_cut_df <- as.data.frame(contrast_cut)
save_table_csv_html(contrast_cut_df, "contrast_fof_by_moderator_cutpoints")

# Optional stratified plots by Sex or Age median split (descriptive only)
...
# Not changing main model, but could create descriptive plots if needed.

# 10. Precision context: detectable interaction effect -----------------------

# Approximate detectable beta for interaction: |beta| ≈ (z_alpha/2 + z_power) * SE_beta
alpha <- 0.05
target_power <- 0.8
...
# We cannot know exact numerical results in advance, but we can construct
# a template that reflects the likely pattern of findings.
...
writeLines(results_text, con = file.path("outputs", "results_addendum_finnish.txt"))
