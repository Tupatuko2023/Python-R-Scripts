...
compute_jn_region_binary <- function(model, alpha = 0.05, mod_var = "cComposite_Z0",
  ...
  if (length(int_coef_name) != 1L) {
    stop(
      "Could not find interaction term between FOF_status1 and moderator in the model.",
      call. = FALSE
    )
  }
...
# data, jossa ei ole puuttuvia arvoja kyseisissä muuttujissa
...
# 4.2 ANCOVA model with interaction (clean quartile model; no continuous Composite_Z0 to avoid collinearity with Comp_Quartile)
...
# Sensitivity: continuous moderator only (continuous moderation analogue of the quartile model).
