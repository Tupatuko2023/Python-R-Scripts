...
# File tag: K24_TABLE2A.V1_delta-by-test-fof-frailty.R
# Purpose: Table 2A per-test delta regressions with FOF and frailty in same model
#          using analysis_plan canonical variable names.
#
...
output_html <- if (!is.na(cli$output_html) && nzchar(cli$output_html)) cli$output_html else file.path(outputs_dir, "table2A_delta_by_test_with_frailty.html")
output_csv <- if (!is.na(cli$output_csv) && nzchar(cli$output_csv)) cli$output_csv else file.path(outputs_dir, "table2A_delta_by_test_with_frailty.csv")
...
append_artifact(
  label = "table2A_delta_by_test_with_frailty_csv",
  kind = "table_csv",
  path = output_csv,
  n = nrow(table_df),
  notes = "K24 Table 2A delta-by-test with joint FOF+frailty model"
)

if (!requireNamespace("gt", quietly = TRUE)) stop("Package 'gt' required for HTML output.")
tbl_gt <- gt::gt(table_df) %>%
  gt::tab_header(
    title = gt::md("**Table 2A - Delta by test with FOF + frailty**"),
    subtitle = paste0("frailty_mode=", cli$frailty_mode, ", include_balance=", cli$include_balance)
  )
gt::gtsave(tbl_gt, filename = output_html)
append_artifact(
  label = "table2A_delta_by_test_with_frailty_html",
  kind = "table_html",
  path = output_html,
  n = nrow(table_df),
  notes = "K24 Table 2A HTML"
)
...
append_artifact(
  ...
  kind = "sessioninfo",
  path = session_path,
  notes = "K24 sessionInfo + renv diagnostics"
)
