#!/usr/bin/env Rscript
# ==============================================================================
# K24_TABLE2A - Canonical-only delta-by-test with joint FOF + frailty (V2)
# File tag: K24_TABLE2A.V2_canonical-delta-by-test-fof-frailty.R
# Purpose: Canonical rerun of Table 2A using K15 frailty-augmented RData
#          (preferred) or CSV only when canonical frailty vars are present.
#
# Canonical-only policy:
# - Preferred input: K15 RData with frailty_cat_3 and frailty_score_3.
# - CSV input allowed only if frailty_cat_3 and frailty_score_3 both exist.
# - No morbidity fallback, no score->cat derivation in V2.
#
# Outputs:
...
parse_cli <- function(args) {
  ...
  if (!out$frailty_mode %in% c("both", "cat", "score")) stop("Invalid --frailty_mode. Use both/cat/score.")
  out
}
...
choose_input_path <- function(cli_input) {
  ...
  if (is.na(hit) || !nzchar(hit)) stop("Input data not found. Provide --input=<K15_RDATA_PATH>")
  normalizePath(hit, winslash = "/", mustWork = TRUE)
}

load_input_data <- function(path) {
  ...
  if (ext %in% c("rdata", "rda")) {
    ...
    if ("analysis_data" %in% objs) {
      cand <- get("analysis_data", envir = e)
      if (inherits(cand, c("data.frame", "tbl_df", "tbl"))) d <- cand
    }
    if (is.null(d)) {
      dfs <- objs[vapply(objs, function(nm) inherits(get(nm, envir = e), c("data.frame", "tbl_df", "tbl")), logical(1))]
      if (length(dfs) == 0) stop("No data.frame/tibble object found in RData input.")
      d <- get(dfs[1], envir = e)
    }
    return(list(data = d, input_kind = "RData", frailty_source = "K15_RData"))
  }

  if (ext %in% c("csv", "txt")) {
    d <- readr::read_csv(path, show_col_types = FALSE)
    need <- c("frailty_cat_3", "frailty_score_3")
    miss <- setdiff(need, names(d))
    if (length(miss) > 0) {
      stop("K24 V2 canonical-only: CSV input missing canonical frailty vars: ", paste(miss, collapse = ", "),
           ". Use K15 frailty-augmented .RData as --input.")
    }
    return(list(data = d, input_kind = "CSV", frailty_source = "canonical_csv"))
  }

  stop("Unsupported input extension: ", ext, ". Use .RData/.rda or canonical CSV with frailty vars.")
}
...
fmt_mean_sd_n <- function(x, n) sprintf("N=%d, %s", n, fmt_mean_sd(x))
...
fmt_delta_ci_n <- function(x, n) sprintf("N=%d, %s", n, fmt_delta_ci(x))
...
frailty_source <- loaded$frailty_source
fallback_used <- FALSE
placeholder_used <- FALSE

col_id <- first_existing(raw_data, c("id", "ID", "NRO", "Jnro"))
...
col_frailty_score <- first_existing(raw_data, c("frailty_score_3"))
...
if (any(is.na(c(col_id, col_age, col_sex, col_bmi, col_fof, col_frailty_cat, col_frailty_score)))) {
  miss <- c("id", "age", "sex", "BMI", "FOF_status", "frailty_cat_3", "frailty_score_3")[is.na(c(col_id, col_age, col_sex, col_bmi, col_fof, col_frailty_cat, col_frailty_score))]
  stop("K24 V2 canonical-only: missing required columns after mapping: ", paste(miss, collapse = ", "))
}
...
analysis_base <- raw_data %>%
  transmute(
    ...
    frailty_cat_3 = normalize_frailty_cat(.data[[col_frailty_cat]]),
    frailty_score_3 = suppressWarnings(as.numeric(.data[[col_frailty_score]])),
...
if (nlevels(droplevels(analysis_base$frailty_cat_3)) < 2) stop("frailty_cat_3 has <2 levels.")
if (all(is.na(analysis_base$frailty_score_3))) stop("frailty_score_3 is all NA; cannot run score-mode.")
...
for (nm in c("MWS", "FTSST", "SLS", "HGS", "HGS (Women)", "HGS (Men)")) {
  ...
  model_df <- d %>% select(delta, FOF_status, frailty_cat_3, frailty_score_3, all_of(covars))

  cat_keep <- c("delta", "FOF_status", "frailty_cat_3", covars)
  model_df_cat <- model_df %>% select(all_of(cat_keep)) %>% tidyr::drop_na()
  fit_cat <- lm(as.formula(paste("delta ~ FOF_status + frailty_cat_3 +", paste(covars, collapse = " + "))), data = model_df_cat)
  fof_cat <- extract_term(fit_cat, "FOF_statusFOF")
  frailty_cat_p <- frailty_overall_p(fit_cat, "frailty_cat_3")
  cat_contrasts <- frailty_cat_contrasts(fit_cat)
  n_model_cat <- as.integer(stats::nobs(fit_cat))

  rows_cat[[length(rows_cat) + 1L]] <- tibble(
    Outcome = nm,
    Frailty_Mode = "cat",
    ...
    FOF_Beta_CI = ifelse(is.na(fof_cat$beta), "", sprintf("%.3f [%.3f, %.3f]", fof_cat$beta, fof_cat$lcl, fof_cat$ucl)),
    P_FOF = fmt_p(fof_cat$p),
    P_Frailty_Overall = fmt_p(frailty_cat_p),
    Frailty_Contrasts = cat_contrasts,
    Model_N = n_model_cat,
    frailty_source = frailty_source,
    fallback_used = fallback_used
  )

  score_keep <- c("delta", "FOF_status", "frailty_score_3", covars)
  model_df_score <- model_df %>% select(all_of(score_keep)) %>% tidyr::drop_na()
  fit_score <- lm(as.formula(paste("delta ~ FOF_status + frailty_score_3 +", paste(covars, collapse = " + "))), data = model_df_score)
  fof_score <- extract_term(fit_score, "FOF_statusFOF")
  frailty_score_term <- extract_term(fit_score, "frailty_score_3")
  n_model_score <- as.integer(stats::nobs(fit_score))

  rows_score[[length(rows_score) + 1L]] <- tibble(
    Outcome = nm,
    Frailty_Mode = "score",
    Balance_Included = cli$include_balance,
    N_without = n_without,
    ...
    Without_FOF_Delta = fmt_delta_ci_n(g0$delta, n_without),
    With_FOF_Delta = fmt_delta_ci_n(g1$delta, n_with),
    FOF_Beta_CI = ifelse(is.na(fof_score$beta), "", sprintf("%.3f [%.3f, %.3f]", fof_score$beta, fof_score$lcl, fof_score$ucl)),
    P_FOF = fmt_p(fof_score$p),
  ...
  compare_rows[[length(compare_rows) + 1L]] <- tibble(
    Outcome = nm,
    N_model_score = n_model_score,
    N_model_cat = n_model_cat,
    AIC_score = AIC(fit_score),
    AIC_cat = AIC(fit_cat),
    FOF_beta_score = fmt_num(fof_score$beta),
    FOF_p_score = fmt_p(fof_score$p),
    ...
    Frailty_score_dist_all = score_dist_string(model_df$frailty_score_3),
    Frailty_score_dist_model = score_dist_string(model_df_score$frailty_score_3),
    Agreement_Flag = agreement_flag(frailty_score_term$beta, frailty_score_term$p, frail_pre$beta, frail_pre$p, frail_fra$beta, frail_fra$p),
    frailty_source = frailty_source,
    fallback_used = fallback_used
  )
}

cat_df <- bind_rows(rows_cat)
score_df <- bind_rows(rows_score)
audit_df <- bind_rows(cat_df, score_df)
compare_df <- bind_rows(compare_rows)

paper_cat <- cat_df %>%
  filter(Outcome != "HGS") %>%
  select(Outcome, Frailty_Mode, Balance_Included, N_without, N_with,
         Without_FOF_Baseline, With_FOF_Baseline, Without_FOF_Delta, With_FOF_Delta,
         FOF_Beta_CI, P_FOF, P_Frailty_Overall, Model_N, frailty_source, fallback_used)

paper_score <- score_df %>%
  filter(Outcome != "HGS") %>%
  select(Outcome, Frailty_Mode, Balance_Included, N_without, N_with,
         Without_FOF_Baseline, With_FOF_Baseline, Without_FOF_Delta, With_FOF_Delta,
         FOF_Beta_CI, P_FOF, Frailty_Score_Beta_CI, P_Frailty_Overall, Model_N, frailty_source, fallback_used)

f_cat_html <- file.path(outputs_dir, "table2A_paper_ready_canonical_cat_v2.html")
f_cat_csv <- file.path(outputs_dir, "table2A_paper_ready_canonical_cat_v2.csv")
f_score_html <- file.path(outputs_dir, "table2A_paper_ready_canonical_score_v2.html")
f_score_csv <- file.path(outputs_dir, "table2A_paper_ready_canonical_score_v2.csv")
f_audit <- file.path(outputs_dir, "table2A_audit_canonical_v2.csv")
f_compare <- file.path(outputs_dir, "table2A_cat_vs_score_compare_canonical_v2.csv")
f_prov <- file.path(outputs_dir, "K24_frailty_provenance_v2.txt")
f_session <- file.path(outputs_dir, "sessionInfo_v2.txt")

readr::write_csv(paper_cat, f_cat_csv)
readr::write_csv(paper_score, f_score_csv)
readr::write_csv(audit_df, f_audit)
readr::write_csv(compare_df, f_compare)

if (!requireNamespace("gt", quietly = TRUE)) stop("Package 'gt' required for HTML output.")

cat_gt <- gt::gt(paper_cat) %>%
  gt::tab_header(
    title = gt::md("**Table 2A canonical (cat) V2**"),
    subtitle = paste0("Input=", input_kind, "; frailty_source=", frailty_source, "; fallback_used=FALSE")
  )
score_gt <- gt::gt(paper_score) %>%
  gt::tab_header(
    title = gt::md("**Table 2A canonical (score) V2**"),
    subtitle = paste0("Input=", input_kind, "; frailty_source=", frailty_source, "; fallback_used=FALSE")
  )

gt::gtsave(cat_gt, filename = f_cat_html)
gt::gtsave(score_gt, filename = f_score_html)

crosscheck_ok <- nrow(paper_cat) == 5 && nrow(paper_score) == 5
prov_lines <- c(
  "K24 canonical frailty provenance v2",
  paste0("timestamp=", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
  paste0("script_version=V2"),
  paste0("input_path=", input_path),
  paste0("input_kind=", input_kind),
  paste0("frailty_cat_source=", frailty_source),
  paste0("frailty_score_source=", frailty_source),
  paste0("fallback_used=", fallback_used),
  paste0("crosscheck_ok=", crosscheck_ok),
  paste0("placeholder_used=", placeholder_used),
  paste0("rows_paper_cat=", nrow(paper_cat)),
  paste0("rows_paper_score=", nrow(paper_score)),
  paste0("rows_audit=", nrow(audit_df)),
  paste0("rows_compare=", nrow(compare_df))
)
writeLines(prov_lines, con = f_prov)

session_lines <- capture.output(sessionInfo())
if (requireNamespace("renv", quietly = TRUE)) {
  session_lines <- c(session_lines, "", "---- renv diagnostics ----", capture.output(renv::diagnostics()))
}
writeLines(session_lines, con = f_session)

append_artifact("table2A_paper_ready_canonical_cat_v2_csv", "table_csv", f_cat_csv, nrow(paper_cat), "K24 V2 canonical cat paper-ready CSV")
append_artifact("table2A_paper_ready_canonical_cat_v2_html", "table_html", f_cat_html, nrow(paper_cat), "K24 V2 canonical cat paper-ready HTML")
append_artifact("table2A_paper_ready_canonical_score_v2_csv", "table_csv", f_score_csv, nrow(paper_score), "K24 V2 canonical score paper-ready CSV")
append_artifact("table2A_paper_ready_canonical_score_v2_html", "table_html", f_score_html, nrow(paper_score), "K24 V2 canonical score paper-ready HTML")
append_artifact("table2A_audit_canonical_v2_csv", "qc_table_csv", f_audit, nrow(audit_df), "K24 V2 canonical audit CSV with frailty provenance fields")
append_artifact("table2A_cat_vs_score_compare_canonical_v2_csv", "qc_table_csv", f_compare, nrow(compare_df), "K24 V2 canonical cat vs score compare CSV")
append_artifact("K24_frailty_provenance_v2_txt", "qc_text", f_prov, notes = "K24 V2 canonical frailty provenance")
append_artifact("sessionInfo_v2", "sessioninfo", f_session, notes = "K24 V2 sessionInfo + renv diagnostics")
