...
* **Thesis working title:** *Detecting and Monetising Fall Risks* (11 Oct 2025 research plan).
...
* **FOF (analysis variable):** Binary yes/no status.
  *Repo standard name:* `FOF_status`
  *Repo convention:* Currently maps raw `kaatumisenpelkoOn` {0,1} → `FOF_status` with reference `nonFOF`, but the raw column name and coding must be confirmed against ingest/data_dictionary before changes.
